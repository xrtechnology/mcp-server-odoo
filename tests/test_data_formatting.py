...
from mcp_server_odoo.formatters import RecordFormatter, DatasetFormatter
    ...
    def test_format_various_field_types(self):
        ...
        try:
            ...
            if product_ids:
                ...
                bool_fields = [k for k, v in fields_meta.items() 
                             if v.get('type') == 'boolean' and k in record]
                if bool_fields:
                    field = bool_fields[0]
                    if record[field]:
                ...
                m2o_fields = [k for k, v in fields_meta.items() 
                ...
                date_fields = [k for k, v in fields_meta.items() 
                             if v.get('type') in ('date', 'datetime') and k in record and record[k]
