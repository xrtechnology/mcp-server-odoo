...
class RecordFormatter:
    ...
    OMIT_FIELDS = {
        ...
        'write_uid', 'create_uid', 'message_follower_ids',
    ...
    def format_record(self, record: Dict[str, Any], 
        ...
        if relation_fields:
            ...
            for field_name, field_value, field_meta in relation_fields:
                lines.extend(self._format_relation_field(
                    field_name, field_value, field_meta, indent_level + 1
                ))
        
        return '\n'.join(lines)
    
    def format_list(self, records: List[Dict[str, Any]], 
                   fields_metadata: Optional[Dict[str, Dict[str, Any]]] = None) -> str:
        ...
        lines = [
            f"{'='*60}",
            f"{self.model} Records ({len(records)} found)",
            f"{'='*60}",
            ""
        ]
    ...
    def _format_field_value(self, field_name: str, value: Any, 
                           field_meta: Dict[str, Any], indent_level: int) -> str:
    ...
    def _format_relation_field(self, field_name: str, value: Any,
                             field_meta: Dict[str, Any], indent_level: int) -> List[str]:
        ...
        # One2many and Many2many fields
            ...
            if value and isinstance(value, list):
                ...
                if count <= self.max_related_items and isinstance(value[0], dict):
                    ...
                    lines.append(f"{indent}  (Showing count only - too many items to display inline)")
            else:
...
class DatasetFormatter:
    ...
    def format_search_results(self, records: List[Dict[str, Any]], 
