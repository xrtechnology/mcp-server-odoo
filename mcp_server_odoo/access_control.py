...
class AccessController:
    ...
    def _make_request(self, endpoint: str, timeout: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            if e.code == 401:
                raise AccessControlError("Invalid API key for access control")
            elif e.code == 403:
                raise AccessControlError("Access denied to MCP endpoints")
            elif e.code == 404:
                raise AccessControlError(f"Endpoint not found: {endpoint}")
            else:
                raise AccessControlError(f"HTTP error {e.code}: {e.reason}")
        except urllib.error.URLError as e:
            raise AccessControlError(f"Connection error: {e.reason}")
        except json.JSONDecodeError as e:
            raise AccessControlError(f"Invalid JSON response: {e}")
        except Exception as e:
            raise AccessControlError(f"Request failed: {e}")
