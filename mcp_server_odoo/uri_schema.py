...
def parse_uri(uri: str) -> OdooURI:
    ...
    if not model:
        raise URIValidationError(f"Invalid model name: empty model name")
    ...
    try:
        ...
        raise URIValidationError(f"Invalid record ID: {record_id_str}")
...
def build_uri(model: str, operation: str, record_id: Optional[int] = None,
              domain: Optional[str] = None, fields: Optional[List[str]] = None,
              limit: Optional[int] = None, offset: Optional[int] = None,
              order: Optional[str] = None, ids: Optional[List[int]] = None) -> str:
...
def build_search_uri(model: str, domain: Optional[str] = None,
                     fields: Optional[List[str]] = None, limit: Optional[int] = None,
                     offset: Optional[int] = None, order: Optional[str] = None) -> str:
    """Build a search URI for the given model and parameters.
    
    This is a convenience function for building search URIs.
    """
    return build_uri(model, "search", domain=domain, fields=fields,
...
def _parse_int_parameter(value: Optional[str], param_name: str) -> Optional[int]:
    ...
    try:
        ...
        raise URIValidationError(f"Invalid {param_name} value: {value}")
...
def _parse_ids_parameter(ids_str: Optional[str]) -> Optional[List[int]]:
    ...
    try:
        ...
        raise URIValidationError(f"Invalid IDs parameter: {ids_str}")